...
class Console {
    ...
    protected function _actionKick() {
        ...
        header(
                sprintf('location: ./admin.php?server=%s&tube=%s', $this->_globalVar['server'], urlencode($this->_globalVar['tube'])));
        exit();
    }
    ...
    protected function _actionLoadSample() {
        ...
        if (isset($_GET['redirect'])) {
            ...
            header(sprintf('location: ./admin.php?server=%s&tube=%s', $this->_globalVar['server'], urlencode($this->_globalVar['tube'])));
        }
        exit();
    }
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                if (isset($_POST['jobdata']) && isset($_POST['name']) && isset($_POST['tubes'])) {
                    ...
                    if ($storage->saveJob($job)) {
                        header('location: ./admin.php?action=manageSamples');
                    } else {
    ...
    private function moveJobsFromTo($server, $tube, $state, $destTube) {
        ...
        header(sprintf('location: ./admin.php?server=%s&tube=%s', $server, urlencode($destTube)));
    }
    ...
    private function moveJobsToState($server, $tube, $state, $destState) {
        ...
        header(sprintf('location: ./admin.php?server=%s&tube=%s', $server, urlencode($tube)));
    }
